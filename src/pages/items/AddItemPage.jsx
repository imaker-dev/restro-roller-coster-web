  ...
  return (
    <>
              ...
              <AccordionSection title="Pricing & Options" icon={CURRENCY.ICON}>
                <div className="space-y-6">
                  ...
                  <ToggleField
                    ...
                    onChange={(value) =>
                      formik.setFieldValue("hasAddons", value)
                    }
                    activeColorClass="bg-emerald-600"
                    disabled={formik.values.isOpenItem}
                  />
      ...
      <ModalAction
        ...
        title={itemId ? "Confirm Item Update" : "Confirm Item Creation"}
        description={
          itemId
            ? `You are about to update "${pendingValues?.name}". These changes will be applied immediately.`
            : `You are about to create "${pendingValues?.name}". The item will be added to the selected category and made available for ordering.`
        }
        confirmText={itemId ? "Update" : "Create"}
