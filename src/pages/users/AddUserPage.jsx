...
const AddUserPage = () => {
  ...
  const handleConfirmSubmit = async () => {
      ...
      roles: [
        {
          ...
          outletId: Number(pendingValues.outletId),
        },
      ],

      floors: (pendingValues.floors || []).map((floorId, index) => ({
        floorId: Number(floorId),
        outletId: Number(pendingValues.outletId),
        isPrimary: index === 0,
      })),
    ...
    if (!userId) {
      ...
      payload.pin = pendingValues.pin;
    }
    if (userId) {
      if (pendingValues.password?.trim()) {
        payload.password = pendingValues.password;
      }
      if (pendingValues.pin?.trim()) {
        payload.pin = pendingValues.pin;
      }
    }
  ...
  return (
    <>
      ...
      <ModalAction
        ...
        description={
          userId
            ? `Are you sure you want to update ${pendingValues?.name || "this staff member"} as ${roles?.find((r) => String(r.id) === String(pendingValues?.roleId))?.name || "N/A"}? Their role, permissions, and outlet access will be modified.`
            : `Are you sure you want to create ${pendingValues?.name || "this staff member"} as ${roles?.find((r) => String(r.id) === String(pendingValues?.roleId))?.name || "N/A"}? They will have access based on the selected role and outlet.`
        }
        theme="warning"
        confirmText={userId ? "Update Staff" : "Create Staff"}
        cancelText="Go Back"
